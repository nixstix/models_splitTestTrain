...
weights <- get_weights(MOFAobject, views = "all", factors = "Factor14", scale = F)
a= weights$geneExp
ggplot(a, aes(x=Factor14)) +
    geom_density() 
...
```{r}
b= weights$ab
ggplot(b, aes(x=Factor14)) +
    geom_density() 


c= weights$cytokineL
ggplot(c, aes(x=Factor14)) +
    geom_density() 


d= weights$cell_freq
ggplot(d, aes(x=Factor14)) +
    geom_density() 
d['Freq.Monocytes', ]
...
rnk = MOFAobject3@expectations$W$geneExp[,'Factor14']
...
```{r}
weights <- get_weights(MOFAobject3, views = "all", factors = "Factor14", scale = F)
weights.scale  <- get_weights(MOFAobject3, views = "geneExp", factors = "Factor14", scale = T)
...
```{r}
weights <- get_weights(MOFAobject3, views = "cytokineL", scale = F)
plot(weights$cytokineL['CytokineL.P10145', ])
plot(weights$cytokineL['CytokineL.P01375', ])

plot(weights$cytokineL['CytokineL.P22301', ])
