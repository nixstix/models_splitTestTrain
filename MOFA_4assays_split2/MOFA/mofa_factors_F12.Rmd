...
weights <- get_weights(MOFAobject, views = "all", factors = "Factor12", scale = F)
a= weights$geneExp
ggplot(a, aes(x=Factor12)) +
    geom_density() 
...
```{r}
b= weights$ab
ggplot(b, aes(x=Factor12)) +
    geom_density() 


c= weights$cytokineL
ggplot(c, aes(x=Factor12)) +
    geom_density() 


d= weights$cell_freq
ggplot(d, aes(x=Factor12)) +
    geom_density() 
d['Freq.Monocytes', ]
...
rnk = MOFAobject3@expectations$W$geneExp[,'Factor12']
...
```{r}
weights <- get_weights(MOFAobject3, views = "all", factors = "Factor12", scale = F)
weights.scale  <- get_weights(MOFAobject3, views = "geneExp", factors = "Factor12", scale = T)
