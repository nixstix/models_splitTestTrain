...
weights <- get_weights(MOFAobject, views = "all", factors = "Factor10", scale = F)
a= weights$geneExp
ggplot(a, aes(x=Factor10)) +
    geom_density() 
...
```{r}
b= weights$ab
ggplot(b, aes(x=Factor10)) +
    geom_density() 


c= weights$cytokineL
ggplot(c, aes(x=Factor10)) +
    geom_density() 


d= weights$cell_freq
ggplot(d, aes(x=Factor10)) +
    geom_density() 
d['Freq.Monocytes', ]
...
rnk = MOFAobject3@expectations$W$geneExp[,'Factor10']
...
```{r}
weights <- get_weights(MOFAobject3, views = "all", factors = "Factor10", scale = F)
weights.scale  <- get_weights(MOFAobject3, views = "geneExp", factors = "Factor10", scale = T)
